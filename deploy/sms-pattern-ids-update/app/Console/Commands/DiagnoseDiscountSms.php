...
class DiagnoseDiscountSms extends Command
{
    ...
    public function handle(SmsService $sms): int
    {
        ...
        if ($this->option('send')) {
            ...
            $this->info("Sending live issued pattern {$issued} to {$to} ...");
            $result = $sms->sendByBaseNumber2($to, $issued, ['تست سیستم', '150,000', Jalalian::now()->format('Y/m/d')]);
            $this->line(json_encode($result, JSON_UNESCAPED_UNICODE));
            if (!($result['success'] ?? false)) {
                return self::FAILURE;
            }
            $this->info('Issued live send OK');

            $this->info("Sending live reminder pattern {$reminder} to {$to} ...");
            $reminderResult = $sms->sendByBaseNumber2($to, $reminder, ['تست سیستم', '150,000']);
            $this->line(json_encode($reminderResult, JSON_UNESCAPED_UNICODE));
            if (!($reminderResult['success'] ?? false)) {
                return self::FAILURE;
            }
            $this->info('Reminder live send OK');
        }
