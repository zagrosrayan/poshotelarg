...
class NextPurchaseDiscountSmsScheduler
{
    ...
    protected function sendDelivery(DiscountSmsDelivery $delivery, Discount $discount): array
    {
        ...
        // بدون جداکننده هزارگان؛ ویرگول متغیرهای SendByBaseNumber2 را خراب می‌کند
        $amount = $this->formatDiscountAmountForSms($discount);
        ...
        if ($delivery->type === DiscountSmsDelivery::TYPE_ISSUED) {
            ...
            // Pattern 500571: {0}=name, {1}=amount/percent, {2}=expires date
        ...
        // Pattern 500779: {0}=name, {1}=amount/percent
        return $this->smsService->sendByBaseNumber2(
            $delivery->recipient,
            (int) $delivery->body_id,
            [$name, $amount]
    ...
    protected function formatDiscountAmountForSms(Discount $discount): string
    {
        if (($discount->discount_type ?? '') === 'percentage') {
            $percent = rtrim(rtrim(number_format((float) $discount->discount_value, 2, '.', ''), '0'), '.');

            return $percent === '' ? '0' : $percent;
        }

        return (string) intval(round((float) $discount->discount_value));
    }
