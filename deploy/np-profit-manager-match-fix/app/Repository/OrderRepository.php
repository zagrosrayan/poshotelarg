...
class OrderRepository
{
    ...
    )
    {
        ...
        if (!empty($discount_code)) {
            try {
                ...
                $discountProfitManagerIds = $this->normalizeProfitManagerIds($discount->profit_manager_ids);
    ...
    {
        $discountProfitManagerIds = $this->normalizeProfitManagerIds($discountProfitManagerIds);
        ...
        foreach ($orderDetails as $order) {
            $food = Food::findOrFail($order['food_id']);
            // از ستون مستقیم بخوان؛ وابستگی به relation گاهی null می‌شود و تخفیف غلط رد می‌شود
            $foodProfitManagerId = (int) ($food->profit_manager_id ?? $food->profitManager?->id ?? 0);

            if ($foodProfitManagerId > 0 && in_array($foodProfitManagerId, $discountProfitManagerIds, true)) {
    ...
    private function shouldApplyDiscountToFood($discount_code, $foodProfitManagerId)
    {
        ...
        $discount = Discount::findByCode($discount_code);
        $allowed = $this->normalizeProfitManagerIds($discount?->profit_manager_ids);

        if (!$discount || empty($allowed)) {
            return true;
        }

        return in_array((int) $foodProfitManagerId, $allowed, true);
    }

    /**
     * SQL Server/JSON occasionally yields string ids; normalize to int list.
     */
    private function normalizeProfitManagerIds($ids): array
    {
        if ($ids === null || $ids === '' || $ids === []) {
            return [];
        }
