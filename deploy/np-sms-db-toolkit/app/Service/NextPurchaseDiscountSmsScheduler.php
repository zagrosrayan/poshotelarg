...
class NextPurchaseDiscountSmsScheduler
{
    ...
    public function dispatchDue(?Carbon $now = null): array
    {
        ...
        if ($settings && $settings->sms_enabled === false) {
            $stats['cancelled'] += DiscountSmsDelivery::cancelPending('sms_disabled');
            $stats['skipped'] = 1;
        ...
        $stats['cancelled'] += DiscountSmsDelivery::cancelPending(
            'reminder_day_passed',
            function ($query) use ($today) {
                $query->where('type', DiscountSmsDelivery::TYPE_REMINDER)
    ...
    protected function sendDelivery(DiscountSmsDelivery $delivery, Discount $discount): array
    {
        ...
        $amount = (string) intval(round((float) $discount->discount_value));
        ...
        if ($delivery->type === DiscountSmsDelivery::TYPE_ISSUED) {
            ...
            // Pattern 500571: {0}=name, {1}=credit amount (rial), {2}=expires date
        ...
        // Pattern 500779: {0}=name, {1}=credit amount (rial)
        return $this->smsService->sendByBaseNumber2(
            $delivery->recipient,
            (int) $delivery->body_id,
            [$name, $amount]
