...
class OrderController extends Controller
{
        ...
        try {
           ...
           $payment_method =  Type::query()->find($request->payment_method);
            ...
            if (in_array($payment_method?->slug, $posPaymentMethods) && !empty($order->reserve_number)) {
                ...
                // مهم: رزرو را از payload هم پاک کن تا در finalize دوباره نوشته نشود
                // و صدور تخفیف خرید بعدی نوع مشتری را درست (Non_resident) تشخیص دهد
                $data['customer_id'] = $customer->id;
                $data['reserve_number'] = null;
            }
    ...
    /**
     * بعد از تکمیل خرید:
     * اگر سفارش با تنظیمات «تخفیف خرید بعدی» سازگار بود → کد تخفیف برای همان مشتری ساخته می‌شود
     * و در صورت روشن بودن sms_enabled پیامک ارسال می‌شود.
     *
     * توجه: فلگ use_next_purchase_discount فقط برای مصرف کد قبلی روی همین فاکتور است
     * و هیچ ربطی به صدور/پیامک تخفیف خرید بعدی ندارد.
     */
    protected function issueNextPurchaseDiscountSms(Order $order, ?string $fallbackPhone = null, ?string $fallbackName = null): void
    {
        ...
        $order->loadMissing(['customer', 'reserve', 'children.food']);

        if (!$order->customer_id && empty($order->reserve_number)) {
            Logger::warning('next_purchase_sms_skip', [
                'order_id' => $order->id,
        ...
        if ($this->customerHasActiveNextPurchaseDiscount($order)) {
            Logger::warning('next_purchase_sms_skip', [
                'order_id' => $order->id,
        ...
        if (!$settings->canApplyForCurrentOrder((float) $order->total_price)) {
            Logger::warning('next_purchase_sms_skip', [
                'order_id' => $order->id,
                'reason' => 'order_amount_below_minimum',
                'total_price' => $order->total_price,
                'minimum' => $settings->minimum_purchase_amount,
            ]);
            return;
        }

        if (!$settings->matchesOrderProfitManagers($order)) {
            $orderPmIds = $order->children
                ->map(fn ($child) => (int) ($child->food?->profit_manager_id ?? 0))
                ->filter()
                ->unique()
                ->values()
                ->all();
            ...
            Logger::warning('next_purchase_sms_skip', [
                ...
                'order_food_profit_manager_ids' => $orderPmIds,
                'allowed' => $settings->profit_manager_ids,
            ]);
            return;
        }

        if (!$settings->matchesCustomerType($order)) {
            Logger::warning('next_purchase_sms_skip', [
                'order_id' => $order->id,
                'reason' => 'customer_type_not_allowed',
                'type' => !empty($order->reserve_number) ? 'resident' : 'Non_resident',
                'allowed' => $settings->target_customer_types,
            ]);
            return;
        }

        $discount = $settings->createDiscount(
        ...
        Logger::warning('next_purchase_discount_created', [
            'order_id' => $order->id,
        ...
        // sms_enabled فقط ارسال پیامک را کنترل می‌کند؛ ساخت تخفیف جداست
    ...
    protected function customerHasActiveNextPurchaseDiscount(Order $order): bool
    {
        if (!$order->customer_id && empty($order->reserve_number)) {
            return false;
        }
